...
@Path("/sudoku")
@RequestScoped
public class SudokuResourceRestService {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public String lookupSudokuStatus(@PathParam("id") long id) {
        return sudokuService.sudokuStatus(id);        
    }
    
    @GET
    @Path("/result/{id:[0-9][0-9]*}")
    @Produces(MediaType.APPLICATION_JSON)
    public SudokuData getSudoku(@PathParam("id") long id) {
